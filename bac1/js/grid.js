			...
			for(var j = 0 ;j < dalu_6[i].length;j++){
				if(dalu_6[i][j] == "none") continue;
					...
					if(dalu_6[i][j][1] != 0 ){
						...
						$(selector+" tr:eq("+col+") td:eq("+j+")").css({"background":"url(./img/dalu_"+dalu_6[i][j][0]+"_he.png) center/100% 100%"});
						...
						$(selector+" tr:eq("+col+") td:eq("+j+")").css({"background":"url(./img/dalu_"+dalu_6[i][j][0]+".png) center/100% 100%"});
					}
	...
	function fill_big_blank(selector){ //大路和主盘填空白图片
		...
		for(i=11;i<51;i++){//大路
			for(j=0;j<6;j++){
  				$(selector+" tr:eq("+i+") td:eq("+j+")").html("<img src='./img/50.png'>");
  				$(selector+" tr:eq("+i+") td:eq("+j+")").css("background","url(./img/50.png) center/100% 100%");
  				num++;
			}
		} 
	}
	...
	function dalu_update(data,selector){   //大路图的更新
		for(var i = 0;i<data.length;i++){
			...
			if(data[i][2][1] != 0 ){
				...
				$(selector+" tr:eq("+row+") td:eq("+col+")").css({"background":"url(./img/dalu_"+data[i][2][0]+"_he.png) center/100% 100%"});
			}
			else
				$(selector+" tr:eq("+row+") td:eq("+col+")").css({"background":"url(./img/dalu_"+data[i][2][0]+".png) center/100% 100%"});
		}
	}
	...
	function count_judge(data){ //统计庄闲和的个数,返回一个庄闲和的数量的数组
      ...
      for(var i = 0 ;i<data.length;i++){
        ...
        if(n == -1)
      	...
      	//   xian_dui++;
		if(data[i] == 3 || data[i] == 4 || data[i] == 7 ||data[i] == 8 || data[i] == 8)	
		xian_dui++;
      }
    ...
    function update_dalu2(data_old,data_new,rowplus,colplus,selector){//大路图的更新2
    	...
    	if(diff_result[2] == "back"){
			$(selector+" tr:eq("+row+") td:eq("+col+")").css({"background":"url(./img/50.png) center/100% 100%"});
			$(selector+" tr:eq("+row+") td:eq("+col+")").html("");
		}
		else{
			if(diff_result.length>0){
				if(diff_result[2][1] != 0 ){
						$(selector+" tr:eq("+row+") td:eq("+col+")").html("<ib>"+diff_result[2][1]+"</ib>");
						$(selector+" tr:eq("+row+") td:eq("+col+")").css({"background":"url(./img/dalu_"+diff_result[2][0]+"_he.png) center/100% 100%"});
				}else{
						$(selector+" tr:eq("+row+") td:eq("+col+")").css({"background":"url(./img/dalu_"+diff_result[2][0]+".png) center/100% 100%"});
						$(selector+" tr:eq("+row+") td:eq("+col+")").html("");
				}
			}
    	}
	...
	// num = 96; /** 珠盘总注数*/
	// selector 标签
	function zhupan_up(data,num,selector){
		var zhupan_data = data;
		if(data.length > num){
			zhupan_data = data.slice(-num,-1);
			zhupan_data.push(data[data.length-1]);
		}
		fill_zhupan(zhupan_data,selector);
		return zhupan_data;
	...
	// num = 32; /** 大路总排数*/
	// selector 标签
	function dalu_up(data,start,num,selector){
		var dalu_data = dalu(data);
		var dalu_6 = convert6(dalu_data);
		var dalu_data = dalu_6;
		if(dalu_6.length > num){
			dalu_data = dalu_6.slice(-num,-1);
			dalu_data.push(dalu_6[dalu_6.length-1]);
		}
		fill_dalu(dalu_data,start,selector);
		return dalu_data;
	}
	// 获取更新的小路
	// num = 32; /** 大路总排数*/
	function getfill_data_up(data,category,num){//生成最后的填入数据，category(1,2,3)分别表示三种模式
		var dalu_data = dalu(data);
	 	var xiaolu_data = other(dalu_data,category);
		// 截取后面的路子
		var xiaolu_6 = convert6(xiaolu_data);
		var xiaolu_data2 = xiaolu_6;
		if(xiaolu_6.length > num){
			xiaolu_data2 = xiaolu_6.slice(-num,-1);
			xiaolu_data2.push(xiaolu_6[xiaolu_6.length-1]);
		}
		// 截取后面的路子
	 	xiaolu_data = smart_img(xiaolu_data2);//小路
	 	return xiaolu_data;
	}
	// start = 0; /** 开始位置*/
	function fill_img_blank_up(start,selector){//填1像素空白图片
		var num = 0;
		for(var i=start;i<51;i++){//填1像素空白图片 右边下部分
			for(var j=6;j<12;j++){
				$(selector+" tr:eq("+i+") td:eq("+j+")").html("<img src='./img/50.png'><img src='./img/50.png'><img src='./img/50.png'><img src='./img/50.png'>");
				num++;
			}
	...
	// start = 0; /** 开始位置*/
	function fill_big_blank_up(start,selector){ //大路和主盘填空白图片
		var num = 0; //主盘
	...
	// num = 32; /** 大路总排数*/
	// start = 0; /** 开始位置*/
	function fill_smail_up(data,start,num,selector){//填所有的数据并把各个路图的数据返回
		var xiaolu_data = getfill_data_up(data,1,num);
		var zhonglu_data = getfill_data_up(data,2,num);
		var weilu_data = getfill_data_up(data,3,num);
		//-----------------------填图---------------------------------------------
		fill_img_blank_up(start,selector);
		fill_big_blank_up(start,selector);
		//**************后三个路图***********************
		fill_img(xiaolu_data,start,6,"dayanzai",selector);
		fill_img(zhonglu_data,start,9,"xiaolu",selector);
		fill_img(weilu_data,Math.floor(num/2),9,"xiaoqiang",selector);
		//*******************************************
		var dalu_data = dalu_up(data,start,num,selector)
		//-----------------------填图end----------------------------------
		var save = new Array();
		save.push(dalu_data,xiaolu_data,zhonglu_data,weilu_data);
		return save;
	}
	// 画底板
	...
	// 横着
	function fill_di_1(tab){
		var div = '';
		var w = $(document).width();
		var td_w = (w/16).toFixed(2);
		for( var i = 1;i <= 16; i++){
			div += `<tr class="left-tr">
						<td class="left-td"></td>
						<td class="left-td"></td>
						<td class="left-td"></td>
						<td class="left-td"></td>
